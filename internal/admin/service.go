...
// NewService builds the admin gRPC service over the anime service, session manager, and OpenAI
// config (an empty key disables résumé tailoring).
func NewService(a *anime.Service, s *Sessions, openAIKey, openAIModel string) *Service {
	return &Service{anime: a, sessions: s, openAIKey: openAIKey, openAIModel: openAIModel}
}
...
func (s *Service) TailorResume(ctx context.Context, req *sitepb.TailorRequest) (*sitepb.Resume, error) {
	if s.openAIKey == "" {
		return nil, status.Error(codes.FailedPrecondition, "tailoring disabled — set OPENAI_API_KEY")
	}
	...
	tailored, err := resume.Tailor(ctx, s.openAIKey, s.openAIModel, jobText, resume.Data())
	if err != nil {
